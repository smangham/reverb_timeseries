...
def import_caramel(caramel_spectra_times_file, caramel_spectra_file):
    """
    ...
    caramel_times = ap.table.Table.read(caramel_spectra_times_file, format='ascii',
                                        names=['time', 'value', 'error'])
    caramel_times['time'].unit = u.s
    ...
    caramel_spectra['value'] = np_lines[1]  # Dummy for base
    ...
    for time_index in range(len(caramel_times)):
        print(time_index, '/', len(caramel_times))
        time = caramel_times['time'].quantity[time_index].to(ucds.MJD)
        spectra_index = 2 + (time_index * 2)  # The first 2 lines are header & wave, rest are spectra
        caramel_spectra['value {}'.format(time)] = np.array(lines[spectra_index])
        caramel_spectra['error {}'.format(time)] = np.array(lines[spectra_index+1])
        caramel_spectra['value {}'.format(time)].meta['name'] = 'CARAMEL'
        caramel_spectra["value {}".format(time)].meta['time'] = time

    return caramel_times, caramel_spectra
